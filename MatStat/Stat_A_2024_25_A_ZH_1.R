...
quantile(pontszamok, 0.75) # 75. percentilis

#b)
# A korrigált szórásnégyzet torzítatlan becslése a valóságos szórásnégyzetnek

#c)
# Ferdeség
# - => Bal oldali asszimetria: Az átlagtól lefelé eltérő értékek nagyobb arányban fordulnak elő, mint a pozitív irányba
...
# - => A normális eloszlásnál lapultabb
...
# 2. feladat: elégséges statisztika
# A paramétert tartalmazó része csak az elégséges statisztikán keresztül függjön az xi mintaelemektől

#3. feladat

mu<-10 # két paraméter ~ normális elsozlás
sigma<-3
n<-c(10, 20, 40, 80, 160)
n_szim<-10000
for (i in n){
  ...
  for (j in 1:n_szim){
    minta<-rnorm(i, mu, sigma) # random normális elsoszlásból
    sigma_becs_1<-c(sigma_becs_1, sd(minta)^2) # a mintából hozzáfűzi a korrigált tapasztalati szórs négyzetet
    sigma_becs_2<-c(sigma_becs_2, (sd(minta)*(i-1)/i)^2) # visszakonvertálja a sima tapasztalati szórással
  }

  # a sigma becsült átlagos értékein
  print(mean(sigma_becs_1)) # nézi, hogy torzítatlan e a becslés, közelíti e a sigma-t^2
...
#5. feladat
# Normális eloszlásnál ugyan arra vezet a MaxLikely mint a Momentum módszer
