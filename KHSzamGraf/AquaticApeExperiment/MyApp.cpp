...
void CMyApp::Render()
{
	...
	glm::vec3 fw = normalize(pos2 - pos1);
	glm::vec3 up = { 0,1,0 };
	glm::vec3 sd = normalize(cross(fw, up));
	glm::vec3 lup = cross(sd, fw);

	//glm::mat4 rot = { {-sd, 0}, {lup, 0}, {fw, 0}, {0, 0, 0, 1} };

	glm::mat4 rot = glm::lookAt(current_pos, pos2, glm::vec3(0, 1, 0));
	...
	glm::mat4 matWorld = glm::inverse(rot);
	...
	glProgramUniform1f(m_programAxis, ul(m_programAxis, "mult"), 0.5);

	glm::mat4 posWorld = glm::translate(m_controlPoints[0]);

	glProgramUniformMatrix4fv(m_programAxis, ul(m_programAxis, "world"), 1, GL_FALSE, glm::value_ptr(posWorld));
	...
	glProgramUniform1f(m_programAxis, ul(m_programAxis, "mult"), 1.0);
	...
	glDrawArrays(GL_LINES, 0, 6);
		...
		glDisable(GL_DEPTH_TEST);
	glProgramUniform1f(m_programAxis, ul(m_programAxis, "mult"), 0.5);
	glm::mat4 posWorld2 = glm::translate(m_controlPoints[1]);
	glProgramUniformMatrix4fv(m_programAxis, ul(m_programAxis, "world"), 1, GL_FALSE, glm::value_ptr(posWorld2));
	glProgramUniformMatrix4fv(m_programAxis, ul(m_programAxis, "viewProj"), 1, GL_FALSE, glm::value_ptr(m_camera.GetViewProj()));
